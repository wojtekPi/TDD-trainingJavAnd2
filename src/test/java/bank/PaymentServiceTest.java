...
public class PaymentServiceTest {
    ...
    private Object[][] paramsForTestingTranseringAmount() {
        return new Object[][]{{0, 100, 200, -200, 300, EUR}, {-50, -40, 70, -120, 30, EUR}, {0, 0, 100, -100, 100, EUR}, {-5, 0, -5, 0, -5, EUR}};
    }

    private Object[][] paramsForTestingTranseringCurrency() {
        return new Object[][]{{PLN, EUR, PLN}, {USD, EUR, USD}, {EUR, PLN, EUR}, {USD, PLN, USD}};
    }
    ...
    public void shouldThrowIllegalArgumentExceptionWithProperMessageWhenNotEnoughMoneyUsingAssertJ() {
        ...
        assertThatExceptionOfType(IllegalArgumentException.class).isThrownBy(() -> testedObject.transferMoney(from, to, new Instrument(EUR, 200))).withMessage(SORRY_TEXT);
    }
